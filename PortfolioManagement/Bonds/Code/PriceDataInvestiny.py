...
class PriceDataInvestiny:
    ...
    def getPriceByIsin(self, isin: str, date: datetime = datetime.today()) -> float:
        """This function accepts an isin and a date for which the price of the bond will be retrieved and returned"""
        try:
    ...
    def getPriceHistory(self, isin: str, fromDate: date, toDate: date) -> pd.DataFrame:
        ...
        toDate, fromDate = self.__convertDatetimesToDateStringsForInvestPy(
            toDate, fromDate
        )

        try:
            historicalData = historical_data(
                self.__getInvestingIdBySearchAssetsApi(isin), fromDate, toDate
            )
            mappedHistoricalData = []
            for i in range(len(historicalData["date"])):
                mappedHistoricalData.append(
                    {
                        "isin": isin,
            ...
            priceHistoryResults = self.__sqlConnection.execute(
                getPriceHistoryByDatabaseSqlStatement
            )
            fetchedPriceHistory = priceHistoryResults.fetchall()
            fetchedPriceHistory = list(
                map(lambda entry: (isin, entry[0], entry[1]), fetchedPriceHistory)
            )
            return pd.DataFrame(fetchedPriceHistory, columns=["isin", "price", "date"])

    def __getPriceByIsinViaInvestingIdCSV(
        self, isin: str, date: datetime = datetime.today()
    ) -> float:
        ...
        try:
            ...
            priceData = historical_data(
                investing_id=investingId, from_date=fromDate, to_date=date
            )["open"][0]
    ...
    def __convertDatetimesToDateStringsForInvestPy(
        self, date: datetime, fromDate: date = None
    ) -> str:
    ...
    def __getInvestingId(self, isin: str) -> int:
        investingIDsDataFrame = pd.read_csv(
            filepath_or_buffer="/home/PortfolioManagement/Resources/InvestingIDs.csv"
        )

        investingRecord = investingIDsDataFrame[
            (investingIDsDataFrame["isin"] == isin)
            & (investingIDsDataFrame["currency"] == "EUR")
        ].head(1)
    ...
    def __getPriceByIsinViaSearchAssets(
        self, isin: str, date: datetime = datetime.today()
    ) -> float:
