...
class BondIndicators:
    ...
    def getProfitOrLossDataFrame(self, date: date = date.today()) -> pd.DataFrame:
        """This function accepts a date for which the profit or loss dataframe should be calculated.
        It returns a dataframe containing the following columns 'isin', 'P/L~Profit Or Loss', 'date'
        for all active bond positions in the portfolio"""
    ...
    def getDepotDataFrame(
        self, date: datetime = date.today(), depot: str = "%%"
    ) -> pd.DataFrame:
        """This function accepts a date for which the 'activeIsins', 'amount of Bonds' and 'value of this position' should be calculated and a depot if you want to filter by depot.
        It returns a dataframe containing the relevant information for your depot"""
        depotDataFrame = []

        for isin in self.getActiveIsins(date, depot):
            profitOrLossData = self.__getProfitOrLossData(isin, date)
            depotDataFrame.append(
                {
                    "isin": isin,
                    "amountOfBonds": profitOrLossData[
                        "totalAmountOfBondsForThisPosition"
                    ],
                    "valueOfThisPosition": profitOrLossData["valueOfPosition"],
                }
            )

        return pd.DataFrame(depotDataFrame)

    def getPriceHistoryDataFrame(
        self, isin: str, dateFrom: date, dateTo: date = date.today()
    ):
        """This function accepts an isin & a from and to date for which the pricehistory shall be retrieved"""
        return self.priceService.getPriceHistory(isin, dateFrom, dateTo)

    def getActiveIsins(
        self, date: date = "CURRENT_DATE", depot: str = "'%%'"
    ) -> list[str]:
        ...
        fetchedActiveIsins = list(
            map(lambda isinTuple: isinTuple[0], activeIsinResult.fetchall())
        )
        return fetchedActiveIsins

    def getProfitOrLossForAPosition(
        self, isin: str, date: date = date.today()
    ) -> float:
        """This function accepts an isin & a date for which the profit or loss at the given date is calculated"""
        profitOrLossData = self.__getProfitOrLossData(isin, date)

        return (
            profitOrLossData["valueOfPosition"]
            - profitOrLossData["totalSumOfPurchases"]
        )
    ...
    def __getProfitOrLossData(self, isin, date):
        ...
        valueOfPosition = (
            self.priceService.getPriceByIsin(isin) * totalAmountOfBondsForThisPosition
        )
        return {
            "totalSumOfPurchases": totalSumOfPurchases,
            "valueOfPosition": valueOfPosition,
            "totalAmountOfBondsForThisPosition": totalAmountOfBondsForThisPosition,
        }

    def calculateProfitOrLossForASale(self, transactionId: int):
        """This function accepts a transactionId and calculates the profit or loss for the sale"""
        ...
        getReferencedTransactionDataSQLStatement = (
            f'Select * from transaction where "transactionID" = {transactionId}'
        )

        transactionResult = self.__sqlConnection.execute(
            getReferencedTransactionDataSQLStatement
        )
        ...
        allPurchasesList = self.__getListOfPurchasesWhichAreHeld(
            allPurchasesList, allSalesList
        )

        totalProfit = 0

        for purchase in allPurchasesList:
            totalProfit += min(saleToCalculate.amount, purchase.amount) * (
                saleToCalculate.price - purchase.price
            )
    ...
    def __getListOfPurchasesWhichAreHeld(
        self, allPurchasesList: list, allSalesList: list
    ) -> list:
        ...
        allPurchasesListIterator = filter(
            lambda purchase: (purchase.amount > 0), allPurchasesList
        )
