...
class Price:
    def __init__(
        self,
        sqlConnection,
        bondIndicators: BondIndicators,
        priceService: PriceDataInvestiny,
    ):
        self.__sqlConnection = sqlConnection
        self.__bondIndicators = bondIndicators
        self.__priceService = priceService

    def manuallyInsertPriceIntoDatabase(
        self, isin: str, price: float, date: date = date.today()
    ):
    ...
    def __storePricesForActivePositions(
        self, fromDate: date = None, toDate: date = date.today()
    ):
        ...
        if fromDate is None:
            latestDateResult = self.__sqlConnection.execute(
                'SELECT max("priceDate")+1 AS priceDate from "Price"'
            )
        ...
        for isin in self.__bondIndicators.getActiveIsins(fromDate):
            priceDataHistoryDataFrame = self.__priceService.getPriceHistory(
